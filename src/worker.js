...
export default {
  ...
  async scheduled(event,env,ctx){if(isBudapest08())ctx.waitUntil(run(env));}
      ...
      errors.push(`Binance HTTP ${r.status}`);
    }catch(e){errors.push(`Binance: ${e.message}`)}
    break;
  }

  if(all.length) return dedupe(all);

  // Kraken fallback is useful for live operation; historical long-range
  // backtests remain Binance-only to keep candle definitions identical.
  throw Error(`Historical data unavailable for ${symbol}/${tf}: ${errors.join(" | ")}`);
}
